...
namespace Fithub1
{
    public partial class Biking : PhoneApplicationPage
    {
        List<bikingData> bikingData;
        bikingData todaybiking;
        ...
        private void pushup_b_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                * Today
                */
                if (DateTime.Today.CompareTo(todaybiking.date) == -1)
                {
                    todaybiking.date = DateTime.Today;
                    todaybiking.distance = distanceNow;
                    todaybiking.calories = calories;
                }
                else
                {
                    todaybiking.distance += distanceNow;
                ...
                IsolatedStorageSettings.ApplicationSettings["biking_today"] = todaybiking;

                /*
                 * Menyimpan  statistik biking
                 * bikingData
                 */
                bikingData.Add(new bikingData(DateTime.Now, distanceNow, totalDuration.ToString(@"hh\:mm\:ss"), avgspeed, avgpace, calories));
                IsolatedStorageSettings.ApplicationSettings["biking_data"] = bikingData;
        ...
        {
            // ambil biking_data 
            if (IsolatedStorageSettings.ApplicationSettings.Contains("biking_data"))
            {
                bikingData = (List<bikingData>)IsolatedStorageSettings.ApplicationSettings["biking_data"];
            }
            else
            {
                bikingData = new List<bikingData>();
            }

            // ambil stats today
            if (IsolatedStorageSettings.ApplicationSettings.Contains("biking_today"))
            {
                todaybiking = (bikingData)IsolatedStorageSettings.ApplicationSettings["biking_today"];
            }
            else
            {
                todaybiking = new bikingData(DateTime.Today, 0.0, 0.0);
            }
